...
/* Inflate a miniz stream starting with start_buf if non-NULL,
...
int spng_decode_scanline(spng_ctx *ctx, void *out, size_t len)
{
    ...
    if(ret == SPNG_EOI)
    {
        if(ctx->cur_chunk_bytes_left) /* miniz stream ended before an IDAT chunk boundary */
...
const char *spng_strerror(int err)
{
    switch(err)
    {
        ...
        case SPNG_EZLIB: return "miniz error";
        ...
        case SPNG_EZLIB_INIT: return "miniz init error";
